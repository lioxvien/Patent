...
; (function ($w) {
    ...
    var nodeClick=function(nodeId,event){return nodeId,event};
    ...
    jm.prototype = {
        ...
        click_handle: function (e) {
            ...
            if (isexpander) {
                ...
                nodeClick(this.view.get_binded_nodeid(element),e);
            }
        },
    ...
    jm.graph_canvas.prototype = {
        ...
        _bezier_to: function (ctx, x1, y1, x2, y2) {
            ctx.beginPath();
            ctx.moveTo(x1, y1);
            ctx.bezierCurveTo(x1 + (x2 - x1)*1/3 , y1, x1, y2, x2, y2);
            ctx.stroke();
        },
    ...
    jm.view_provider.prototype = {
        ...
        expand_size: function () {
            ...
            var min_width = min_size.w + this.opts.hmargin * 0.5;
            var min_height = min_size.h + this.opts.vmargin * 0.5;
        ...
        create_node_element: function (node, parent_node) {
            ...
            if (node.isroot) {
                ...
                if (node.parent.isroot){
                    ...
                    let lastParent=node.parent.parent;
                    if (lastParent.isroot){
                        index=node.parent.id %5;
                    }else{
                        let lasterParent=lastParent.parent;
                        if (lasterParent.isroot){
                            index=lastParent.id %5;
                        }else{
                            if (node.id>100000){
                                let deptId= node.id.toString().substring(2,4);
                                index=parseInt(deptId);
                            }else{
                                index=999;
                            }
                        }
        ...
        zoomOut: function (val) {
            if (val instanceof Number){
                console.log("============")
                return this.setZoom(this.actualZoom -val * this.zoomStep);
            }
            return this.setZoom(this.actualZoom - this.zoomStep);
        },
        ...
        add_css_node(){
            var nodes=  this.jm.mind.root.children;
            var level=1;
            for (var nodeid in nodes) {
    ...
    // quick way
    jm.show = function (options, mind,ick) {
        nodeClick=ick;
